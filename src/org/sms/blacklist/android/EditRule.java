...
public class EditRule extends Activity {
	...
	private View.OnClickListener mOnSaveClickListener = new View.OnClickListener() {
		public void onClick(View v) {
			...
			if (type == Constants.TYPE_BLOCKED_NUMBER_REGEXP
					|| type == Constants.TYPE_BLOCKED_KEYWORD_REGEXP) {
				try {
					Pattern.compile(rule);
				} catch (RuntimeException e) {
					Toast toast = Toast.makeText(EditRule.this,
							getString(R.string.syntax_error),
							Toast.LENGTH_SHORT);
					toast.show();
					return;
				}
			...
			if (rule.length() > 0) {
				...
				Toast toast = Toast.makeText(EditRule.this,
						getString(R.string.invaild_value), Toast.LENGTH_SHORT);
				toast.show();
			}
		}
	};
	...
	private View.OnClickListener rOnDeleteClickListener = new View.OnClickListener() {
		public void onClick(View v) {
			new AlertDialog.Builder(EditRule.this).setIcon(
					android.R.drawable.ic_dialog_alert).setTitle(
					R.string.delete).setMessage(
					getString(R.string.delete_rule_confirm)).setPositiveButton(
